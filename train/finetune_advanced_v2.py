...
# --- 路径定义 ---
# 关键：加载由第一个脚本产出的、在CIFAR-10上预训练好的模型
PRETRAINED_MODEL_PATH = "model/rotnet_cifar10_pretrained.pth" 
FINETUNED_MODEL_PATH = "model/rotation_finetuned_from_cifar.pth" # 新的保存路径
NEW_DATA_DIR = "data/image/"

print("--- 实验：加载CIFAR-10预训练模型，在自定义数据集上微调 ---")
print(f"将要加载模型: '{PRETRAINED_MODEL_PATH}'")
print(f"使用设备: {DEVICE}")
print(f"将使用差异化学习率: Encoder LR = {ENCODER_LR}, Classifier LR = {CLASSIFIER_LR}")
print(f"图片尺寸: {IMAGE_SIZE}x{IMAGE_SIZE}")


# --- 2. 辅助类和自定义Dataset (与您提供的脚本一致) ---
...
class RotationPredictionModel(nn.Module):
    def __init__(self, num_classes=4):
        ...
        # 关键点: 初始化时不加载任何权重(weights=None)，因为我们将从文件加载
        print("模型初始化：编码器权重将由加载的CIFAR-10预训练文件填充。")
...
# --- 4. 数据加载与划分 (已修正) ---
# 关键修正：将数据增强和Tensor转换合并，修复了会导致程序崩溃的错误

# 训练集使用丰富的数据增强，并包含最终的格式转换
...
# 创建训练和验证数据集 (使用修正后的transform)
train_dataset = CustomImageDataset(image_paths=train_paths, transform=train_transform)
val_dataset = CustomImageDataset(image_paths=val_paths, transform=val_transform)
...
# 加载在CIFAR-10上预训练好的模型权重
if os.path.exists(PRETRAINED_MODEL_PATH):
    print(f"--- 正在加载在CIFAR-10上预训练的模型权重 ---")
    # 因为预训练脚本保存的是state_dict本身，所以直接加载即可
    state_dict = torch.load(PRETRAINED_MODEL_PATH, map_location=DEVICE)
    model.load_state_dict(state_dict)
    print("--- 预训练模型加载成功！---")
else:
    print(f"错误：找不到预训练模型 '{PRETRAINED_MODEL_PATH}'。请先运行第一个预训练脚本。")
    exit()
...
for epoch in range(NUM_EPOCHS_FINE_TUNE):
    ...
    correct_train = 0
    total_train = 0
    for images, labels in tqdm(train_loader, desc="训练中", colour="green"):
    ...
    total_val = 0
    with torch.no_grad():
        for images, labels in tqdm(val_loader, desc="验证中", colour="cyan"):
            images, labels = images.to(DEVICE), labels.to(DEVICE)
            outputs = model(images)
            loss = criterion(outputs, labels)
